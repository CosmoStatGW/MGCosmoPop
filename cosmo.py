...
cosmoglob = FlatwCDM(H0=globals.H0GLOB, Om0=globals.Om0GLOB, w0=-1)
...
dLGridGLOB = cosmoglob.luminosity_distance(zGridGLOB).to(globals.which_unit).value
...
def uu(z, Om, w0):
    ...
    '''
    return 70/globals.clight*FlatwCDM(H0=70, Om0=Om, w0=w0).comoving_distance(z).value
...
def j(z, Om, w0):
    ...
    '''
    return 4*np.pi*FlatwCDM(H0=70, Om0=Om, w0=w0).differential_comoving_volume(z).value*(70/globals.clight)**3
...
def z_from_dLGW_fast(r, H0, Om, w0, Xi0, n):
    ...
    if (Om==globals.Om0GLOB) & (w0==-1):
        ...
        cosmo = FlatwCDM(H0=globals.H0GLOB, Om0=Om, w0=w0)
        dLGrid = cosmo.luminosity_distance(zGridGLOB).to(globals.which_unit).value
    z2dL = interpolate.interp1d( dLGrid/H0*globals.H0GLOB*Xi(zGridGLOB, Xi0, n), zGridGLOB, kind='cubic', bounds_error=False, fill_value=(0,0.), assume_sorted=True)
    return z2dL(r)
